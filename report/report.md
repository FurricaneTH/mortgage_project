...
**Privacy handling and rationale:** the assignment materials do not establish whether the supplied loan package is synthetic, so I treated borrower, property, and loan identifiers as potentially sensitive. The pipeline now replaces those values with stable placeholders before writing `result.json`. This preserves evidence locations, document agreement, conflict status, and review decisions while preventing the submitted JSON and report from exposing the source identifiers. Because earlier repository revisions contained the unredacted generated artifacts, I rewrote the final branch history and checked the resulting Git objects for those values. The committed placeholders cannot be compared directly with the source PDF; exact-value verification remains a local review against the authorized source.
